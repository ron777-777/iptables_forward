...
def add_forward(): 
    ...
    forward_port = input('输入本地端口(默认10000):') or '10000'
    print(forward_port)

    forwarded_ip = input('输入被转发ip:')
    print(forwarded_ip)

    forwarded_port = input('输入被转发端口(默认' + forward_port + '):') or forward_port
...
def del_forwarding():
    ...
    if show == 'Chain POSTROUTING (policy ACCEPT 460 packets, 28030 bytes)\npkts bytes target prot opt in out source destination':
        ...
        if output == 'iptables: Bad rule (does a matching rule exist in that chain?).':
            ...
            print('规则已删除')
            choice = input('是否继续删除y/n(默认y):') or 'y'
            if choice == 'y':
                del_forwarding()
            else:
                print('回到主菜单')
...
select = input('iptables端口转发一键管理脚本\n'
               '--------------------\n'
               '1.安装iptables\n'
               '2.清空iptables端口转发\n'
               '--------------------\n'
